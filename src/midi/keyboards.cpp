...
int process([[maybe_unused]]jack_nframes_t nframes, [[maybe_unused]]void* arg)/*{{{*/
{
	...
	should_send_GM_messages = false;
	should_send_PatchChange = false;
	should_send_next_SysExEs = false;
...
void Keyboard::dump_performance(const Performance& _Performance) noexcept {/*{{{*/
	...
	n_prev_SysExEs = 2;
	const jack_midi_data_t to_combi_SysEx[] {0xF0, 0x42, 0x30, 0x7A, 0x4E, 0x00, 0xF7};
	const jack_midi_data_t to_timbre_SysEx[]{0xF0, 0x42, 0x30, 0x7A, 0x4E, 0x01, 0xF7};
	memcpy(prev_SysExEs[0], to_combi_SysEx, SYSEX_LENGTH);
	memcpy(prev_SysExEs[1], to_timbre_SysEx, SYSEX_LENGTH);
...
void Keyboard::dump_scene() noexcept/*{{{*/
{
	...
	// Status (todos en OFF)
	jack_midi_data_t x50_on_off[8][13] = 
	...
	jack_midi_data_t x50_volume[8][13] =
	...
	// Lower Key (C-1)
	jack_midi_data_t x50_lower_key[8][13] = 
	...
	// Upper Key (C-1)
	jack_midi_data_t x50_upper_key[8][13] = 
	...
	// Transposition (0)
	jack_midi_data_t x50_transposition[8][13] = 
	...
	for (std::size_t i {0}; i < 8; ++i)
		memcpy(next_SysExEs[i], x50_on_off[i], SYSEX_LENGTH);
	for (std::size_t i {0}; i < 8; ++i)
		memcpy(next_SysExEs[i + 8], x50_volume[i], SYSEX_LENGTH);
	for (std::size_t i {0}; i < 8; ++i)
		memcpy(next_SysExEs[i + 16], x50_lower_key[i], SYSEX_LENGTH);
	for (std::size_t i {0}; i < 8; ++i)
		memcpy(next_SysExEs[i + 24], x50_upper_key[i], SYSEX_LENGTH);
	for (std::size_t i {0}; i < 8; ++i)
		memcpy(next_SysExEs[i + 32], x50_transposition[i], SYSEX_LENGTH);

	should_send_next_SysExEs = true;
...
void Keyboard::select_page(const enum Page& _Pagina) noexcept {/*{{{*/
	...
	n_prev_SysExEs = 1;
	memcpy(prev_SysExEs[0], pageSysEx[_Pagina], SYSEX_LENGTH);

	should_send_prev_SysExEs;
